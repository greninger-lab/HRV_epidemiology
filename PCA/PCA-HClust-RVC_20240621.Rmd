...
RVCdistdf <- read.csv("GeneticDistanceMatrix_RV-C_20240621.csv", row.names=1) #the pairwise genetic distance dataframe
RVCmetadf <-read.csv("RVC-PCA_metadata_20240621.csv", row.names=1) #the metadata associated to sequences/individuals
...
RVCmetadf$Clinica <- as.factor(RVCmetadf$Symptoms)
...
#svglite(filename = "RVC_HC_PCA_euclidean_TEST.svg")
#plot(RVChc, hang=-1, cex=0.2) 
#rect.hclust(RVChc, k=5, border=2:4) #to see the clusters identified by a rectangle
#dev.off()
```
...
library("AMR") #G-test
RVC_cluster_pre <- merge(RVCmetadf, cut_cmp, by = 0)
RVC_cluster_pre$y<-as.factor(RVC_cluster_pre$y)
RVC_cluster <- RVC_cluster_pre[!is.na(RVC_cluster_pre$Age),]
RVC_cluster_clinica <- RVC_cluster[!is.na(RVC_cluster$Clinica),]
RVC_cluster_zipcode <- RVC_cluster[!is.na(RVC_cluster$ZipCode),]
RVC_cluster_sex <- RVC_cluster[!is.na(RVC_cluster$Sex),]

g.test(RVC_cluster_clinica$Clinica, RVC_cluster_clinica$y)
X1 = pairwiseNominalIndependence(as.matrix(table(RVC_cluster_clinica$Clinica, RVC_cluster_clinica$y)), compare = "column", fisher = FALSE, chisq = FALSE, method = "bonferroni", digits = 3)
cldList(p.adj.Gtest ~ Comparison, data= X1, threshold  = 0.05)
ggplot(RVC_cluster_clinica, aes(x=Clinica, fill=y)) + geom_bar(position = "fill")

g.test(RVC_cluster_zipcode$ZipCode, RVC_cluster_zipcode$y)
X2 = pairwiseNominalIndependence(as.matrix(table(RVC_cluster_zipcode$ZipCode, RVC_cluster_zipcode$y)), compare = "column", fisher = FALSE, chisq = FALSE, method = "bonferroni", digits = 3)
cldList(p.adj.Gtest ~ Comparison, data= X2, threshold  = 0.05)
ggplot(RVC_cluster_zipcode, aes(x=ZipCode, fill=y)) + geom_bar(position = "fill") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

X3 = pairwiseNominalIndependence(as.matrix(table(RVC_cluster_sex$Sex, RVC_cluster_sex$y)), compare = "column", fisher = FALSE, chisq = FALSE, method = "bonferroni", digits = 3)
cldList(p.adj.Gtest ~ Comparison, data= X3, threshold  = 0.05)
ggplot(RVC_cluster_sex, aes(x=Sex, fill=y)) + geom_bar(position = "fill")
...
ggplot(RVC_cluster, aes(x=AgeGroup, fill=y)) + geom_bar(position = "fill") + scale_x_discrete(limits = c("<5y", "5-18y", "19-65y", ">65y"))

RVC_cluster_pre$Year <- as.factor(substr(RVC_cluster_pre$CollectionDate, 1, 4))
X5 = pairwiseNominalIndependence(as.matrix(table(RVC_cluster_pre$Year, RVC_cluster_pre$y)), compare = "column", fisher = FALSE, chisq = FALSE, method = "bonferroni", digits = 3)
cldList(p.adj.Gtest ~ Comparison, data= X5, threshold  = 0.05)
ggplot(RVC_cluster_pre, aes(x=Year, fill=y)) + geom_bar(position = "fill")

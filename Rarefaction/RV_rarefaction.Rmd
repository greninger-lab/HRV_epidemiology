...
#Rarefaction curves.
In 2021 there were 72 genotypes detected within a total of 958 observations, in 2022 there were 48 genotypes with 120 observations.
...
#Loading dataframe:
RV_dataframe <- read_csv("RV-dataframe_20240621.csv")
...
numberRV <- read.csv("numberRV_20240621.csv", row.names=1)
out <- iNEXT(numberRV, q=0, datatype="abundance")
ggiNEXT(out, type=3)

numberRV_period <- read.csv("numberRV_20240621_period.csv", row.names=1)
out <- iNEXT(numberRV_period, q=0, datatype="abundance")
ggiNEXT(out, type=3)

numberRVA <- read.csv("numberRVA_20240621.csv", row.names=1)
outA <- iNEXT(numberRVA, q=0, datatype="abundance")
ggiNEXT(outA, type=3)

numberRVB <- read.csv("numberRVB_20240621.csv", row.names=1)
outB <- iNEXT(numberRVB, q=0, datatype="abundance")
ggiNEXT(outB, type=3)

numberRVC <- read.csv("numberRVC_20240621.csv", row.names=1)
...
#per month for RV-A
ChaoRichness(numberRVA[,c("X2021.04", "X2021.05", "X2021.06", "X2021.07", "X2022.11", "X2022.12")], conf=0.99)

#per month for RV-B
ChaoRichness(numberRVB[,c("X2021.04", "X2021.05", "X2021.06", "X2021.07", "X2022.11", "X2022.12")], conf=0.99)
